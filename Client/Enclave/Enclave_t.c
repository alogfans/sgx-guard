...
typedef struct ms_enclave_ra_create_t {
	sgx_status_t ms_retval;
	int ms_b_pse;
	uint32_t* ms_context;
} ms_enclave_ra_create_t;
...
typedef struct ms_enclave_ra_set_remote_pubkey_t {
	sgx_status_t ms_retval;
	sgx_ec256_public_t ms_pubkey;
} ms_enclave_ra_set_remote_pubkey_t;

typedef struct ms_enclave_seal_aes_key_t {
	sgx_status_t ms_retval;
	uint8_t* ms_aes_key;
	uint32_t ms_sealed_size;
	uint8_t* ms_sealed;
} ms_enclave_seal_aes_key_t;

typedef struct ms_enclave_seal_size_t {
	int ms_retval;
	uint32_t ms_payload_size;
} ms_enclave_seal_size_t;
...
static sgx_status_t SGX_CDECL sgx_enclave_ra_create(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_enclave_ra_create_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_enclave_ra_create_t* ms = SGX_CAST(ms_enclave_ra_create_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint32_t* _tmp_context = ms->ms_context;
	size_t _len_context = sizeof(*_tmp_context);
	uint32_t* _in_context = NULL;
	...
	if (_tmp_context != NULL && _len_context != 0) {
		if ((_in_context = (uint32_t*)malloc(_len_context)) == NULL) {
	...
	ms->ms_retval = enclave_ra_create(ms->ms_b_pse, _in_context);
err:
	if (_in_context) {
		memcpy(_tmp_context, _in_context, _len_context);
		free(_in_context);
	}
...
static sgx_status_t SGX_CDECL sgx_enclave_ra_set_remote_pubkey(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_enclave_ra_set_remote_pubkey_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_enclave_ra_set_remote_pubkey_t* ms = SGX_CAST(ms_enclave_ra_set_remote_pubkey_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = enclave_ra_set_remote_pubkey(ms->ms_pubkey);


	return status;
}

static sgx_status_t SGX_CDECL sgx_enclave_seal_aes_key(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_enclave_seal_aes_key_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_enclave_seal_aes_key_t* ms = SGX_CAST(ms_enclave_seal_aes_key_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	if (_tmp_aes_key != NULL && _len_aes_key != 0) {
		...
		memcpy(_in_aes_key, _tmp_aes_key, _len_aes_key);
	}
	...
	ms->ms_retval = enclave_seal_aes_key(_in_aes_key, _tmp_sealed_size, _in_sealed);
err:
...
static sgx_status_t SGX_CDECL sgx_enclave_seal_size(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_enclave_seal_size_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_enclave_seal_size_t* ms = SGX_CAST(ms_enclave_seal_size_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = enclave_seal_size(ms->ms_payload_size);
...
SGX_EXTERNC const struct {
	...
	8,
	{
		{(void*)(uintptr_t)sgx_enclave_ra_create, 0},
		{(void*)(uintptr_t)sgx_enclave_ra_close, 0},
		{(void*)(uintptr_t)sgx_enclave_ra_set_remote_pubkey, 0},
		{(void*)(uintptr_t)sgx_enclave_seal_aes_key, 0},
		{(void*)(uintptr_t)sgx_enclave_seal_size, 0},
